...
class PDF(FPDF):
    def header(self):
        self.image("bgs/ans_ess.jpg", 0, 0, 8.625)
    ...
    for ans_num in range(round(total_sudo/3)):
        ...
        for row in range(9):
            for column in range(9):
                pdf.cell(w=height, h=height, txt=f"{sudoku_answers[answer_num][row][column]}", border=1, align="C", ln=False)
            pdf.cell(w=sudo_distance + 0.112, h=height, ln=False, border=0)

            for column in range(9):
                pdf.cell(w=height, h=height, txt=f"{sudoku_answers[answer_num + 1][row][column]}", border=1, align="C", ln=False)
            pdf.cell(w=sudo_distance + 0.112, h=height, ln=False, border=0)

            for column in range(9):
                pdf.cell(w=height, h=height, txt=f"{sudoku_answers[answer_num + 2][row][column]}", border=1, align="C", ln=False)
            pdf.cell(w=height, h=height, ln=True)

        pdf.cell(w=1.8, h=0.25, txt=f"Sudoku #{answer_num + 1}",  ln=False, border=0, align="C")
        pdf.cell(w=0.743, h=0.25, txt=f"",  ln=False, border=0)
        pdf.cell(w=1.8, h=0.25, txt=f"Sudoku #{answer_num + 2}",  ln=False, border=0, align="C")
        pdf.cell(w=0.743, h=0.25, txt=f"",  ln=False, border=0)
        pdf.cell(w=1.8, h=0.25, txt=f"Sudoku #{answer_num + 3}",  ln=True, border=0, align="C")
